  ...
  newfiletype(:crontab) do
    ...
    def read
      unless Puppet::Util.uid(@path)
        Puppet.debug _("The #{@path} user does not exist. Treating their crontab file as empty in case Puppet creates them in the middle of the run.")
      ...
      when /are not allowed to/
        Puppet.err _("The %{path} user is not authorized to use cron. Their crontab file is treated as empty in case Puppet modifies e.g. cron.deny or cron.allow in the middle of the run.") % { path: @path }
