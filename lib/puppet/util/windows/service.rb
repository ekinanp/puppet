...
module Puppet::Util::Windows
  ...
  module Service
    ...
    def stop(service_name)
      ...
      valid_initial_states = SERVICE_STATES.keys - [SERVICE_STOP_PENDING, SERVICE_STOPPED]
    ...
    class << self
      ...
      # @param [:Integer] to_state the state that the service will transition to
      def transition_service_state(service_name, valid_initial_states, to_state, &block)
        ...
        open_service(service_name, SC_MANAGER_CONNECT, service_access) do |service|
          status = query_status(service)
          from_state = status[:dwCurrentState]

          unless valid_initial_states.include?(from_state)
            valid_initial_states_str = valid_initial_states.map do |state|
              SERVICE_STATES[state]
            end.join(", ")

            raise Puppet::Error, _("The service must be in one of the %{valid_initial_states} states to perform this transition. It is currently in the %{current_state} state.") % { valid_initial_states: valid_initial_states_str, current_state: SERVICE_STATES[from_state] }
          end
          ...
          # to perform the state transition (the service thread may not even
          # exist).
          # 
          # NOTE: The other pending states SERVICE_CONTINUE_PENDING and
          # SERVICE_PAUSE_PENDING do not have this issue because the service
          # thread should already be running and able to perform the state
          # transition. We could wait on these pending states if we wanted to.
          # However this would be a bad idea, because then our customers would
          # not be able to, for example, stop a service that's been stuck in
          # SERVICE_CONTINUE_PENDING with Puppet. In that case, the service
          # provider may timeout and fail while waiting for the
          # SERVICE_CONTINUE_PENDING transition to finish.
          if UNSAFE_PENDING_STATES.include?(from_state)
            final_state = FINAL_STATES[from_state]
            Puppet.debug _("The service is in the %{pending_state} state, which is an unsafe pending state.") % { pending_state: from_state }
            wait_on_pending_state(service, from_state)
            from_state = final_state
          end

          Puppet.debug _("Transitioning the %{service_name} service from %{from_state} to %{to_state}") % { service_name: service_name, from_state: SERVICE_STATES[from_state], to_state: SERVICE_STATES[to_state] }

          yield service

          Puppet.debug _("Waiting for the transition to finish")
          wait_on_state_transition(service, from_state, to_state)
        end
      rescue => detail
        raise Puppet::Error, _("Failed to transition the %{service_name} service to the %{to_state} state. Detail: %{detail}") % { service_name: service_name, to_state: SERVICE_STATES[to_state], detail: detail }, detail.backtrace
      end
      ...
      # @api private
      # Sends a service control signal to a Windows service
      #
      ...
      def send_service_control_signal(service, signal)
        FFI::MemoryPointer.new(SERVICE_STATUS.size) do |status_ptr|
          ...
          if ControlService(service, signal, status) == FFI::WIN32_FALSE
            raise Puppet::Util::Windows::Error.new(_("Failed to send the %{control_signal} signal to the service. Its current state is %{current_state}. Failed with") % { control_signal: SERVICE_CONTROL_SIGNALS[signal], current_state: SERVICE_STATES[status[:dwCurrentState]] })
          end
        end
      end

      # @api private
      # Waits for a Windows service to transition from one state to
      # another state.
      #
      # @param [:handle] service handle to the service to wait on
      # @param [Integer] from_state the state that the service is transitioning from.
      # @param [Integer] to_state the state that the service is transitioning to
      def wait_on_state_transition(service, from_state, to_state)
        # Get the pending state for this transition
        if to_state != SERVICE_RUNNING
          pending_state = FINAL_STATES.key(to_state)
        elsif from_state == SERVICE_STOPPED
          # to_state == SERVICE_RUNNING
          pending_state = SERVICE_START_PENDING
        else
          # from_state == SERVICE_PAUSED, to_state == SERVICE_RUNNING
        ...
        while elapsed_time <= DEFAULT_TIMEOUT
          ...
          if state == pending_state
            Puppet.debug _("The service transitioned to the %{pending_state} state.") % { pending_state: pending_state }
        ...
        raise Puppet::Error, _("Timed out while waiting for the service to transition from %{from_state} to %{to_state} OR from %{from_state} to %{pending_state} to %{to_state}. The service's current state is %{current_state}.") % { from_state: SERVICE_STATES[from_state], to_state: SERVICE_STATES[to_state], pending_state: SERVICE_STATES[pending_state], current_state: SERVICE_STATES[state] }
      end
      private :wait_on_state_transition

      # @api private
      # Waits for a Windows service to finish transitioning from
      ...
      def wait_on_pending_state(service, pending_state)
        ...
        Puppet.debug _("Waiting for the pending transition to the %{final_state} state to finish.") % { final_state: final_state }
