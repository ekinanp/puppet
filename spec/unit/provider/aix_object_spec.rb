...
describe 'Puppet::Provider::AixObject' do
  ...
  # Clear out the class' current set of attributes
  ...
  describe '.mapping' do
    ...
    shared_examples 'a mapping' do |from, to|
      context "<#{from}> => <#{to}>" do
        ...
        it 'creates the mapping' do
          mapping_info["#{from_suffix}_to_#{to_suffix}".to_sym] = lambda { |x| x.to_s }
          provider.class.mapping(mapping_info)
    
          mappings = provider.class.mappings[to]
          expect(mappings).to include(mapping_info[from])

          mapping = mappings[mapping_info[from]]
          expect(mapping.public_methods).to include(conversion_fn)

          expect(mapping.send(conversion_fn, 3)).to eql('3')
        end
  
        it 'uses the identity function as a conversion function if none is provided' do
          provider.class.mapping(mapping_info)
    ...
    it 'sets the AIX attribute to the Puppet property if it is not provided' do
      mapping_info[:aix_attribute] = nil
      provider.class.mapping(mapping_info)

      mappings = provider.class.mappings[:puppet_property]
      expect(mappings).to include(mapping_info[:puppet_property])
    end
  end
  ...
  describe '.numeric_mapping' do
    let(:mapping_info) do
      ...
      provider.class.numeric_mapping(info)

      info
    end
    let(:aix_attribute) do
      provider.class.mappings[:aix_attribute][mapping_info[:puppet_property]]
    end
    let(:puppet_property) do
      provider.class.mappings[:puppet_property][mapping_info[:aix_attribute]]
    end

    it 'raises an ArgumentError for a non-numeric Puppet property value' do
      value = 'foo'
      expect { aix_attribute.convert_property_value(value) }.to raise_error do |error|
        expect(error).to be_a(ArgumentError)

        expect(error.message).to match(value)
        expect(error.message).to match(mapping_info[:puppet_property].to_s)
      end
    end
...
AIX_OBJECTS
  ...
  describe '.attributes_to_args' do
    ...
    it 'converts the attributes hash to CLI arguments' do
      expect(provider.class.attributes_to_args(attributes)).to eql(
...
bin:2
    ...
    # Add a mapping for the 'id' attribute
    before(:each) do
      provider.class.mapping(
        puppet_property: :gid,
        aix_attribute: :id,
        # We construct our lambda this way to ensure that it's
        # called.
        attribute_to_property: lambda { |x| "+#{x}" }
      )
    end
    ...
    it 'lists all of the objects' do
      ...
      expect(provider.class.list_all).to eql({ 'system' => '+0', 'staff' => '+1', 'bin' => '+2' })
    end
  end
  ...
  describe '#object_info' do
    ...
    it 'collects the Puppet properties' do
      ...
      provider.class.stubs(:parse_aix_objects)
        .with(output)
        .returns({ resource.name => mock_attributes })
  ...
  describe "#create" do
    ...
    it "creates the AIX object with the given AIX attributes + Puppet properties" do
      ...
      provider.expects(:addcmd).with(attributes.merge(property_attributes)).returns('addcmd')
