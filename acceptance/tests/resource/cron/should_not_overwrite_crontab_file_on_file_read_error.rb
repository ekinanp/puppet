test_name "The crontab provider should not overwrite the crontab file when it cannot read it" do
  ...
  agents.each do |agent|
    ...
    step "Set the user's crontab" do
      ...
      assert_match(/\A#{crontab_contents}\z/, run_cron_on(agent, :list, username).stdout.chomp, "Could not set the user's crontab for testing")
    end
...
SCRIPT
    ...
    step "(PUP-9217) Attempt to overwrite the user's crontab file" do
      ...
      #   to overwrite our user's crontab file so that it only contains Cron['second_entry'].
      manifest = [
...
SCRIPT
    ...
    step "(PUP-9217) Verify that Puppet does not overwrite the user's crontab file when it fails to read it" do
      assert_match(/\A#{crontab_contents}\z/, run_cron_on(agent, :list, username).stdout.chomp, "Puppet overwrote the user's crontab file even though it failed to read it")
    end
  end
end
