module Puppet
  module Acceptance
    module AixUtil
      ...
      def assert_object_attributes_on(agent, object_get, object, expected_attributes)
        agent.send(object_get, object) do |result|
...
MANIFEST
      ...
      def run_attribute_management_tests(object_resource, id_property, attributes, changed_attributes)
        ...
        agents.each do |agent|
          ...
          run_errors_when_property_is_passed_as_attribute_step = lambda do
            manifest = object_resource_manifest(
              object_resource,
              name,
              attributes: attributes.merge({ 'id' => '15' })
            )
     
            apply_manifest_on(agent, manifest) do |result|
              assert_match(/Error:.*attributes.*setting.*#{id_property} property.*id attribute.*#{id_property} property/, result.stderr, "specifying a Puppet property as part of an AIX attribute should have errored, but received #{result.stderr}")
            end
          end
          ...
          step "Ensure that Puppet errors if a Puppet property is passed in as an AIX attribute when creating the #{object_resource}" do
            run_errors_when_property_is_passed_as_attribute_step.call
          end
          ...
          step "Ensure that the #{object_resource} can be created with the specified attributes" do
            ...
            assert_object_attributes_on(agent, object_get, name, attributes)
          end
          ...
          step "Ensure that Puppet accepts a hash for the attributes property" do
            # We want to see if Puppet will actually do something with the
            # attributes property, so let's set one of our attributes back
            # to its old value.
            attribute = old_attributes.keys.first
            old_value = old_attributes.delete(attribute)
            attributes[attribute] = old_value
